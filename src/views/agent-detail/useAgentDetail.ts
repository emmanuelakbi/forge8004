...
export function useAgentDetail() {
  ...
  const runSpotGridCycle = async ({
    ...
    if (!runtime || runtime.status === "stopped") {
      ...
      // Bybit-style: immediately buy at market price to fill all buy levels
      // below current price, arming their paired sell levels above.
      const currentPrice = getGridPriceForAsset(runtime.asset, latestMarket);
      const initialBuyResult = performInitialMarketBuy(
        runtime,
        currentPrice,
        timestamp,
      );
      runtime = initialBuyResult.runtime;
      const filledBuyCount = initialBuyResult.filledBuys.length;
      const armedSellCount = initialBuyResult.armedSells.length;

      syncGridRuntimeState(runtime);

      // Persist each initial market buy as a grid buy intent artifact
      for (const [idx, buyLevel] of initialBuyResult.filledBuys.entries()) {
        const pairedSell = initialBuyResult.armedSells[idx];
        if (!pairedSell) continue;
        await persistGridBuyIntent({
          timestamp: timestamp + idx + 1,
          asset: runtime.asset,
          quantity: buyLevel.quantity || 0,
          entryPrice: currentPrice,
          quoteAllocated: buyLevel.quoteAllocated,
          pairedSellPrice: pairedSell.price,
          policy: riskPolicy,
          capitalBefore: currentBalance,
          capitalAfter: currentBalance - buyLevel.quoteAllocated,
          totalTreasury,
          rangeLow: runtime.rangeLow,
        });
      }
  ...
  const handleRegisterOnChain = async () => {
    if (!agentId || !agent || !user || isRegistering) return;
    if (agent.identity.onChain?.tokenId) return; // already registered
    setIsRegistering(true);
    try {
      if (!onChainService.isAvailable()) return;
      ...
      console.error("[OnChain] registerAgent from detail failed:", error);
    } finally {
