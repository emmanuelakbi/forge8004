...
import { resolvedFirebaseConfig } from "@/app/lib/firebase";
import {
...
export default function TopBar({
  ...
  // Clear auth error and close panel when user becomes authenticated
  useEffect(() => {
    if (user) {
      setAuthError(null);
  ...
  useEffect(() => {
    ...
    return onWalletChange((addr) => setWalletAddress(addr));
  }, []);
    ...
    const fetchGas = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setGasPrice(txCostUsd < 0.01 ? "<$0.01" : `~${txCostUsd.toFixed(2)}`);
        } else {
          setGasPrice(
            txCostEth < 0.000001
              ? "<$0.01"
  ...
  const handleConnectWallet = async () => {
    ...
    if (wallets.length > 1) {
      ...
      try {
        const { selectWalletProvider } = await import("@/src/services/wallet");
  ...
  const handleEmailAuth = async () => {
    try {
      ...
      if (authMode === "register") {
        await registerWithEmail(authName, authEmail, authPassword);
      } else {
    ...
    <>
      <header className="relative min-h-20 border-b border-border-subtle bg-obsidian/80 backdrop-blur-xl flex items-center justify-between gap-4 px-4 py-4 md:px-8 sticky top-0 z-40">
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-4">
          ...
          {loading ? (
            <div className="flex items-center gap-2">
              <div className="w-10 h-10 border border-border-subtle bg-zinc-deep animate-pulse" />
            </div>
          ) : user ? (
